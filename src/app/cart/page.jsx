...
const Page = () => {
    ...
    return(
        <div className='mt-16 md:w-[50%] md:ml-[25%] w-[95%] ml-[2.5%]'>
            {cart.length == 0 ? <div className='w-full mt-16 mb-48 text-xl animate-pulse text-center font-["font2"]'>Your Cart Is Empty!</div> :
            cart.map((item) => (
                <div key={item.id}>
                  <div className='block  rounded-xl shadow-md border-slate-400 dark:border-slate-600  bg-slate-400 dark:bg-slate-600 h-32 mt-4'>
                    ...
                    <div className='block'>
                    <div className='p-2 font-semibold font-["font2"] md:text-md text-sm h-20'>{item.title}</div> 
                      <div className='flex'>
                        ...
                        <div className='flex'>
                          <div className='font-["font2"] md:text-md text-sm'>{item.price}</div>
                          <div><X /></div>
                          <div className='font-["font2"] md:text-md text-sm'>{item.quantity}</div>
                          <div><Equal /></div>
                          <div className='font-["font2"] md:text-md text-sm'>{item.price * item.quantity}</div>
                        </div>
                      </div>
                    </div>
            ...
            {cart.length > 0 &&
            <div className='mt-8 text-center w-[50%] ml-[25%] rounded-xl font-semibold font-["font2"] text-white p-2 mb-32 md:text-xl text-lg bg-red-600'>Total: {TotalPrice}</div>}
        </div>
    )
}
